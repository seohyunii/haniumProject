...
public class MedInfoActivity extends AppCompatActivity {
    ...
    private void showMed() {
        ...
        if (cancel) {
            ...
            Getmedtakentime(new MedInfoData(senddate, user_id));
        }
    ...
    // 약 정보 가져오기 api 호출
    private void Getmedtakentime(MedInfoData data) {
        service.userGetMedTime(data).enqueue(new Callback<MedInfoResponse>() {
            @Override
            public void onResponse(Call<MedInfoResponse> call, Response<MedInfoResponse> response) {
                ...
                MedInfoResponse result = response.body();
                ...
                if(result.getLength()==0){
                    data.add("해당 일자 약 복용 정보가 없습니다.");
                }

            }
            @Override
            public void onFailure(Call<MedInfoResponse> call, Throwable t) {
                Toast.makeText(MedInfoActivity.this, "찾기 에러 발생", Toast.LENGTH_SHORT).show();
